...
package SMMID::Role::Site::Exceptions;
use Moose::Role;
...
sub throw {
    ...
    if( @_ > 1 ) {
        ...
        my $exception = SMMID::Exception->new( %args );
...
sub _coerce_to_exception {
    my ( $self, $thing ) = @_;
    return $thing if  blessed($thing) && $thing->isa('SMMID::Exception');
    return SMMID::Exception->new( message => "$thing" );
}
